...
y <- t(y)
m <- t(m)
data <- list("gsets", "y", "n", "listlength", "m", "w", "dsets") # to be passed on to WinBUGS

myinits <-	list(
  list(c = 20.5, a = c(-.003, .63), s = 9.5))  
 
# parameters to be monitored:	
parameters <- c("c","s","t","predpc","a")

# The following command calls WinBUGS with specific options.
# For a detailed description see Sturtz, Ligges, & Gelman (2005).
samples = bugs(data, inits=myinits, parameters,
	 			       model.file ="SIMPLE_2R.txt", n.chains=1, 
...
for (i in 1:howmany){
	predt <- samples$sims.array[,,"a[1]"][[keep[i]]]*wdom+samples$sims.array[,,"a[2]"][[keep[i]]]
