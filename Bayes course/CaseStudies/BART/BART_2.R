...
data <- list("nconds", "ntrials", "p", "options", "d") # to be passed on to WinBUGS
myinits <- list(
...
samples <- bugs(data, inits=myinits, parameters, model.file = "BART_2.txt", 
	         n.chains=2, n.iter=5000, n.burnin=2000, n.thin=1, 
	         bugs.directory=bugsdir, codaPkg=F, debug=T)
# Now the values for the monitored parameters are in the "samples" object, 
# ready for inspection.

gplus.sober <- samples$sims.list$gplus[,1]
gplus.tipsy <- samples$sims.list$gplus[,2]
gplus.drunk <- samples$sims.list$gplus[,3]

beta.sober  <- samples$sims.list$beta[,1]
beta.tipsy  <- samples$sims.list$beta[,2]
beta.drunk  <- samples$sims.list$beta[,3]
