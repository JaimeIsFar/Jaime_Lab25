...
parameters <- c("alpha","beta","z")

# The following command calls WinBUGS with specific options.
# For a detailed description see Sturtz, Ligges, & Gelman (2005).
samples <- bugs(data, inits=myinits, parameters,
	 			model.file ="Psychophysical_2.txt",
...
for (i in 1:8)
{
	...
	for (j in 1:nstim[i])
  {
		ztemp[j,] <- samples$summary[paste("z[",as.character(i),",",as.character(j),"]",sep=""),"mean"]
	}
	zmean[[i]] <- ztemp
}
alpha2	   <- samples$sims.list$alpha
beta2  	   <- samples$sims.list$beta
