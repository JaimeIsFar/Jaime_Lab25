% Pledgers_2, with order restriction
clear;
...
nsamples = 5e5;  %How Many Recorded Samples?
...
for i=1:nchains
    S.thetap(1) = -rand;
    S.thetap(2) = S.thetap(1)/2;
   init0(i) = S;
end

if ~run_model
    load Pledgers_2 samples stats
else
    if ~ sampler
% Use WinBUGS to Sample
[samples, stats] = matbugs(datastruct, ...
    fullfile(pwd, 'Pledgers_2.txt'), ...
    'init', init0, ...
    'nChains', nchains, ...
    ...
    save Pledgers_2 samples stats
end;

%% Analysis
crit = 0;
eps =.001;binsc = [-1+eps/2:eps:0];binse = [-1:eps:0];
...
set(gca,'xlim',[-.025 0],'xtick',[-.02 0 ],'ytick',[]);
