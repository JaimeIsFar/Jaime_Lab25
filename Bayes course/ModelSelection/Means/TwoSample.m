...
datastruct = struct('x',x,'n1',n1,'y',y,'n2',n2);
...
if ~run_model
    load TwoSample samples stats
else
    if ~sampler
        % Use WinBUGS to Sample
        [samples, stats] = matbugs(datastruct, ...
            fullfile(pwd, 'TwoSample.txt'), ...
            'init', init0, ...
            'nChains', nchains, ...
...
ff=1.3;set(gcf,'units','norm','pos',[.2 .2 .4*ff .3*ff],'paperpositionmode','auto');
set(gca,'fontsize',14);
crit = 0;
