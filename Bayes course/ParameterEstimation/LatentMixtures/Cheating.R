...
data <- list("p", "k", "n", "truth") # to be passed on to WinBUGS
myinits <- list(
...
samples = bugs(data, inits=myinits, parameters,
               model.file ="Cheating.txt",
               n.chains=2, n.iter=3000, n.burnin=1000, n.thin=1,
               DIC=T, bugs.directory=bugsdir,
               codaPkg=F, debug=T)
               
samples$summary
pc <- samples$sims.list$pc/p #to get proportion correct
mean(pc)
...
par(cex.main = 1.5, mar = c(5, 6, 4, 5) + 0.1, mgp = c(3.5, 1, 0), cex.lab = 1.5,
    font.lab = 2, cex.axis = 1.3, bty = "n", las=1)
plot(k,samples$mean$z,ylim=c(0,1),xlim=c(0,n), xlab= "Number of Items Recalled Correctly", 
